...
import { Platform, SafeAreaView, ScrollView, TextInput, UIManager, View, StyleSheet, Text } from "react-native";
...
const ContentCreate = () => {
    ...
    const customTitle = (item: Note) => {
        ...
        return <View style={stylesAccordion.viewTitle}>
            <View>
    ...
    const customBody = () => {
        return <View style={stylesAccordion.viewBody}>
            <View style={stylesAccordion.div}></View>
            <View style={stylesAccordion.viewContent}>
                <Text>Mô tả ngắn nội dung<Text style={stylesAccordion.styleDot}> *</Text></Text>
    ...
    const customIcon = (item: Note) => {
        const isItem = listAccordion.find(i => i.id === item.id)
        return <View style={{ ...stylesAccordion.viewIcon, transform: `${isItem && isItem?.isOpen ? 'rotate(90deg)' : 'rotate(0deg)'}`}} >
            {iconArrowDown({ width: "12", height: "12" })}
        </View>
    }
    return <SafeAreaView style={stylesAccordion.container}>
        <ScrollView >
            {DataNote.map(item => (
                <AccordionItem
                    ...
                    onPress={(isOpen) => {
                        ...
                        const index = openAccordionItem.findIndex(i => i.id === item.id);
